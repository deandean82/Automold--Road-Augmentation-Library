...
def brighten(image_dict: dict, brightness_coeff=-1):  ##function to brighten the image
    ...
    for filename, img in image_dict.items():
        ...
        image_RGB_dict[filename] = change_light(img, brightness_coeff_t)
    return image_RGB_dict


def darken(images_dict, darkness_coeff=-1):  ##function to darken the image
    # verify_image(image)
    if (darkness_coeff != -1):
    ...
    for filename, img in images_dict.items():
        ...
        image_RGB_dict[filename] = change_light(img, darkness_coeff_t)
    return image_RGB_dict


def random_brightness(images_dict):
    # verify_image(image)
    image_RGB_dict = {}
    for filename, img in images_dict.items():
        random_brightness_coefficient = 2 * np.random.uniform(0, 1)  ## generates value between 0.0 and 2.0
        image_RGB_dict[filename] = change_light(img, random_brightness_coefficient)
    return image_RGB_dict
...
def add_shadow(image, no_of_shadows=1, rectangular_roi=(-1, -1, -1, -1), shadow_dimension=5):  ## ROI:(top-left x1,y1, bottom-right x2,y2), shadow_dimension=no. of sides of polygon generated
    ...
    if rectangular_roi == (-1, -1, -1, -1):
        ...
        #     y2=image.shape[0]
        # else:
        y1 = list(image.values())[0].shape[0] // 2
        x2 = list(image.values())[0].shape[1]
        y2 = list(image.values())[0].shape[0]

    elif x1 == -1 or y1 == -1 or x2 == -1 or y2 == -1 or x2 <= x1 or y2 <= y1:
        raise Exception(err_invalid_rectangular_roi)
    image_RGB_dict = {}
    image_list_dict = image
    for filename, img in image_list_dict.items():
        output = shadow_process(img, no_of_shadows, x1, y1, x2, y2, shadow_dimension)
        image_RGB_dict[filename] = output
    return image_RGB_dict
...
def add_snow(image, snow_coeff):
    ...
    for filename, img in images_dict.items():
        ...
        real_snow_coeff = snow_coeff * 255 / 2
        real_snow_coeff = snow_coeff + 255 / 3
        if (is_list(image)):
            image_RGB = []
            image_list = image
            for img in image_list:
                output = snow_process(img, real_snow_coeff)
                image_RGB.append(output)
        else:
            image_snow_dict[filename] = snow_process(img, real_snow_coeff)
    return image_snow_dict
...
def add_rain(image, slant=-1, drop_length=20, drop_width=1, drop_color=(200, 200, 200), rain_type='None'):  ## (200,200,200) a shade of gray
    ...
    image_RGB_dict = {}
    image_list_dict = image
    imshape = list(image.values())[0].shape
    if slant_extreme == -1:
...
def add_fog(image_dict, fog_coeff=-1):
    # verify_image(image)
    image_fog_dict = {}
    for filename, img in image_dict.items():
        imshape = img.shape
        if (fog_coeff != -1):
            ...
            if not kernel_size%2:
                kernel_size += 1
            image_fog_dict[filename] = cv2.blur(img, (kernel_size, kernel_size))
        else:
            imshape = img.shape
            if fog_coeff == -1:
                fog_coeff_t = random.uniform(0.3, 1)
            ...
            haze_list = generate_random_blur_coordinates(imshape, hw)
            for haze_points in haze_list:
                image = add_blur(img, haze_points[0], haze_points[1], hw, fog_coeff_t)
            image_fog_dict[filename] = cv2.blur(image, (hw // 10, hw // 10))

    return image_fog_dict
...
def add_sun_process(image, no_of_flare_circles, flare_center, src_radius, x, y, src_color):
    ...
    for i in range(no_of_flare_circles):
        ...
        cv2.circle(overlay, (int(x[r]), int(y[r])), rad * rad * rad,
                   (random.randint(max(src_color[0] - 50, 0), src_color[0]), random.randint(max(src_color[1] - 50, 0), src_color[1]), random.randint(max(src_color[2] - 50, 0), src_color[2])), -1)
        cv2.addWeighted(overlay, alpha, output, 1 - alpha, 0, output)
    ...
    for filename, img in image_dict.items():
        ...
        if (is_list(image)):
            ...
            if (speed_coeff == -1):
                ...
                count_t = int(15 * speed_coeff)
            image_speed_dict[filename] = apply_motion_blur(img, count_t)

    return image_speed_dict
...
def correct_exposure(image):
    ...
    for filename, img in image_list_dict.items():
        cnt += 1
        image_RGB_dict[filename] = exposure_process(img)
    return image_RGB_dict
